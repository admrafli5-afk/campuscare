const allowedClinicRoles = [
  ...
  function getStatusBadgeClass(status) {
    if (status === "completed" || status === "health_check" || status === "sick_letter") {
  ...
  function renderTable(items) {
    ...
    items.forEach((item) => {
      ...
      const status = item.status || item.type || "health_check";
    ...
    if (!keyword) {
      ...
      filteredHistoryData = historyData.filter((item) => {
        const combinedText = [
          ...
          getActionTaken(item),
          mapHistoryStatus(item.status || item.type),
        ]
          .join(" ")
          .toLowerCase();
  ...
  async function loadPatientHistory() {
    ...
    try {
      const result = await apiRequest("/health-checks/history", {
        method: "GET",
      });
      ...
      if (!result.success) {
        ...
        showError(
          result.message ||
            "Endpoint riwayat pasien belum tersedia dari backend. Halaman siap untuk integrasi CP4."
        );
      ...
      showError(
        "Tidak dapat memuat riwayat pasien. Pastikan backend berjalan atau endpoint CP4 tersedia."
      );
